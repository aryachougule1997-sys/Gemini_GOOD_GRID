...
-- Task submissions table for completion tracking
CREATE TABLE task_submissions (
    ...
    status VARCHAR(50) DEFAULT 'PENDING', -- PENDING, UNDER_REVIEW, APPROVED, REJECTED, NEEDS_REVISION
...
-- Manual review queue for complex verifications
...
-- Feedback system for task completion
CREATE TABLE task_feedback (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    submission_id UUID REFERENCES task_submissions(id) ON DELETE CASCADE,
    from_user_id UUID REFERENCES users(id) ON DELETE CASCADE, -- Who gave the feedback
    to_user_id UUID REFERENCES users(id) ON DELETE CASCADE, -- Who received the feedback
    rating INTEGER CHECK (rating >= 1 AND rating <= 5),
    feedback_text TEXT,
    feedback_type VARCHAR(50) DEFAULT 'COMPLETION', -- COMPLETION, QUALITY, COMMUNICATION, etc.
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE reward_distributions (
    ...
    trust_score_change INTEGER DEFAULT 0,
    rwis_awarded INTEGER DEFAULT 0,
...
CREATE INDEX idx_verification_queue_assigned ON verification_queue(assigned_reviewer_id);
CREATE INDEX idx_task_feedback_submission ON task_feedback(submission_id);
